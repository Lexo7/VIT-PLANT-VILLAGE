"""
Training script for Vision Transformer models.
"""
...
def main():
    parser = argparse.ArgumentParser(description='Train Vision Transformer models for plant disease detection')
    parser.add_argument('--data_dir', type=str, default='data/plant_village/processed',
                        help='Path to processed dataset')
    parser.add_argument('--model', type=str, default='vit_base_patch16_224', 
                        choices=['vit_base_patch16_224', 'vit_small_patch16_224', 'custom_vit'],
                        help='ViT model to train')
    ...
    if args.model == 'custom_vit':
        model = CustomViT(num_classes=num_classes, img_size=args.image_size)
    else:
        model = VisionTransformer(num_classes=num_classes, model_name=args.model)
    ...
    # Tensorboard writer
    writer = SummaryWriter(log_dir=os.path.join(args.log_dir, f'vit_{args.model}'))
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if val_acc > best_acc:
            best_acc = val_acc
            best_checkpoint_path = os.path.join(args.output_dir, f'best_vit_{args.model}.pth')
            save_checkpoint(model, optimizer, epoch, val_loss, val_acc, best_checkpoint_path)
            print(f'  New best model saved with accuracy: {best_acc:.2f}%')
        
        # Save checkpoint every 10 epochs
        if (epoch + 1) % 10 == 0:
            checkpoint_path = os.path.join(args.output_dir, f'vit_{args.model}_epoch_{epoch}.pth')
            save_checkpoint(model, optimizer, epoch, val_loss, val_acc, checkpoint_path)
    
    # Save final model
    final_checkpoint_path = os.path.join(args.output_dir, f'final_vit_{args.model}.pth')
    ...
    config_path = os.path.join(args.output_dir, f'config_vit_{args.model}.json')
