...
def main():
    ...
    # Tensorboard writer
    writer = SummaryWriter(log_dir=os.path.join(args.log_dir, f'cnn_{args.model}'))
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if val_acc > best_acc:
            best_acc = val_acc
            best_checkpoint_path = os.path.join(args.output_dir, f'best_cnn_{args.model}.pth')
            save_checkpoint(model, optimizer, epoch, val_loss, val_acc, best_checkpoint_path)
            print(f'  New best model saved with accuracy: {best_acc:.2f}%')
        
        # Save checkpoint every 10 epochs
        if (epoch + 1) % 10 == 0:
            checkpoint_path = os.path.join(args.output_dir, f'cnn_{args.model}_epoch_{epoch}.pth')
            save_checkpoint(model, optimizer, epoch, val_loss, val_acc, checkpoint_path)
    
    # Save final model
    final_checkpoint_path = os.path.join(args.output_dir, f'final_cnn_{args.model}.pth')
    ...
    config_path = os.path.join(args.output_dir, f'config_cnn_{args.model}.json')
